...
#
# DESCRIPTION:
# Runs the appropriate script for adding/removing an entry for the new VM to 
# the hosts file

# the location of the host list addition/removal scripts
SCRIPTS_DIR="/usr/local/bin"

if $(echo "$@" | grep -qw "\-\-run") && ! $(echo "$@" | grep -qw "\-\-cluster"); then
    tmp_vm_info=`mktemp /tmp/tbmcc.XXXXXXX`
    $SCRIPTS_DIR/add-host-entry $tmp_vm_info &
    exec $_RUNJAVA "$@" | tee $tmp_vm_info
    exit 0
elif $(echo "$@" | grep -qw "\-\-terminate") || $(echo "$@" | grep -qw "\-\-save"); then
    vm_handle=$(echo "$@" | tr ' ' '\n' |  sed -n '/--handle/{n;p}')
